...
// establish session access to blocked sites 
...
function scanURL(aSubject, uri) {
  if (blacklist.enabled && blacklist.sites_enabled && typeof ignoredSchemes[uri.scheme] === "undefined") {
    ...
    if (blacklist.advanced_limitNetAccess) {
      ...
      let msg = (blacklist.advanced_showDetails) ? stringBundle.GetStringFromName("internetBlockEnabled") : "";
    ...
    if (match !== -1) {
      aSubject.cancel(Components.results.NS_ERROR_FAILURE); // Cancel the request
      let webNav = aSubject.notificationCallbacks.getInterface(nsIWebNavigationIface);
      let msg = (blacklist.advanced_showDetails) ? stringBundle.GetStringFromName("addressMatched") + " \"" + URI_spec.substr(match,20) + "\u2026\"" : "";
      filteredNode(webNav, uri, msg, false);
      return;
    }
  }
...
function contentListener(event) {
  ...
  if (scanContentAllowed) {
    ...
    let cf = new contentFilter();
    cf.init(doc, elements);
  }

  if (profanitylist.enabled) {
    let pf = new profanityFilter();
    pf.init(doc, elements);
  }
...
contentFilter.prototype.i = 0;
contentFilter.prototype.t = "";
contentFilter.prototype.MillisecondsPauseBetweenBatches = 3;
contentFilter.prototype.NodesPerBatch = 20;

contentFilter.prototype.init = function(doc, elements) {
  this.doc = doc;
...
contentFilter.prototype.traceDoc = function(ms) {
  ...
  timer.initWithCallback(
    (function(_that){
      return function(){_that.filter(_that);};
...
contentFilter.prototype.filter = function(_that) {
  ...
  while ((el=_that.elements.snapshotItem(_that.i++)) && (loopCount <= _that.NodesPerBatch)) {
    ...
    if (pn === null || pn.nodeType === 9 || pn.nodeType === 11 ||
      (pn.nodeType === 1 && 
        !(pn instanceof nsIDOMHTMLScriptElementIface ||
          pn instanceof nsIDOMHTMLStyleElementIface))) {
      data += el.data + " ";
    }
    loopCount++;
  }
  ...
  if (el != null) {
    ...
    if (match !== -1){
      let msg = (blacklist.advanced_showDetails) ? stringBundle.GetStringFromName("contentMatched") + " \"" + _that.t.substr(match,20) + "\u2026\"" : "";
...
profanityFilter.prototype.i = 0;
profanityFilter.prototype.MillisecondsPauseBetweenBatches = 3;
profanityFilter.prototype.NodesPerBatch = 20;

profanityFilter.prototype.init = function(doc, elements) {
  this.doc = doc;
  this.elements = elements;
  this.traceDoc(0);
};
...
profanityFilter.prototype.traceDoc = function(ms) {
  var timer = Cc["@mozilla.org/timer;1"].createInstance(nsITimerIface), _that = this;
  timer.initWithCallback( (function(_that) { return function() { _that.filter(_that); }; })(_that), ms, nsITimerIface.TYPE_ONE_SHOT);
};
...
function filteredNode(doc, URI, msg, isContent) {
  ...
  blockedURI = URI;
  let msg = (blacklist.advanced_customWarning) ? blacklist.advanced_customWarningMsg + " " + msg : stringBundle.GetStringFromName("unavailablePage") + " " + msg;
  let button = [{
