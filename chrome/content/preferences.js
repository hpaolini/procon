...
const prefwindow =
{
	...
	load: function()
	{
		...
		var promptVals = (window.arguments) ? window.arguments[1] : {};
	...
	{
		var opener = window.opener, promptVals = { title: promptTitle, message: promptMsg };
		if (opener) {
...
const io =
{
	...
	reset: function()
	{
		...
		{
			let Prefs = Cc["@mozilla.org/preferences-service;1"].getService(Ci.nsIPrefService);
			if (Prefs.prefHasUserValue("nglayout.initialpaint.delay")
						&& Prefs.getIntPref("nglayout.initialpaint.delay") == 100000) {
				Prefs.clearUserPref("nglayout.initialpaint.delay");
			}
		}
...
const mainPane =
{
	...
	{
		var bool = (el) ? el.checked : document.getElementById("pref-blacklist-enabled").value;
	...
	setPassword : function(el)
	{
		...
		check = {value: false}; //XXX: need to pass an object for the checkbox, even if hidden
...
const subscriptionsPane =
{
	...
	saveSubscriptionsUrls : function(listBox)
	{
		...
		updateButton.disabled = (!len) ? true : false;
